    ...
    <div class="settings-group card mb-4">
        ...
        <div class="card-body">
            <div v-for="(field, key) in group.fields" :key="key">
                <FormField :model-value="localValues[key]" @update:model-value="updateValue(key, $event)"
                    :type="field.type || 'text'" :label="getLabel(field.label, key)"
                    :description="getDescription(field.description)" :placeholder="getPlaceholder(field.placeholder)"
                    :rules="field.rules || []" :error="errors[key]" :disabled="readonly || field.disabled"
                    :required="field.required" :options="field.options" :min="field.min" :max="field.max"
                    :step="field.step" :rows="field.rows" :accept="field.accepted || field.accept" :icon="field.icon"
                    :searchable="field.searchable" :monospace="field.monospace" />
